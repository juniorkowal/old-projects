...
class ReplayBuffer():
    ...
    def sample(self, k=1):
        ...
        indices = np.random.choice(np.arange(len(self.buffer), dtype = int), samples_count)

        samples = []
        for indice in indices:
            samples.append(self.buffer[indice])
...
class Agent:
    def __init__(self, **kwargs):
        ...
        self.buffer = ReplayBuffer(self.NUM_PARAMS * 10)
    ...
    def update_weights(self, state, state_prime, R, A_index):
        coded_state = self.coder[state]
        coded_state_prime = self.coder[state_prime]

        q_values = self.neural_approx.predict(coded_state)
        q_pred = q_values[A_index]
        q_target = R + self.gamma * np.max(self.neural_approx.predict(coded_state_prime))
    ...
    def start_training(self):
        ...
        for episode in range(episode_num):
            ...
            while (krok < 1000) & (czy_wahadlo_przewrocilo_sie == 0):
                ...
                q_error = self.update_weights(state, state_prime, R, A_index)
                q_errors += q_error
