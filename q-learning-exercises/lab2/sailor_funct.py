...
def sailor_test(reward_map, Q, num_of_episodes):
    ...
    return str(np.mean(sum_of_rewards))


# drawing map of rewards and strategy using arrows
def draw(reward_map, Q, name, r, iterat, e):
    num_of_rows, num_of_columns = reward_map.shape
    ...
    f.suptitle(f"Końcowa średnia suma nagród: {r},\n liczba iteracji: {iterat}, \n osiągnięty błąd: {e}")
    f.text(0.5, 0.03, f'{name}', ha='center', va='bottom')
