...
class Agent():
    ...
    def start_training(self):
        ...
        self.alpha = 0.2            # wsp.szybkosci uczenia(moze byc funkcja czasu)
        ...
        for episode in range(episode_num): # DLA KAŻDEGO EPIZODU
            ...
            while (krok < 1000) & (czy_wahadlo_przewrocilo_sie == 0):
                ...
                delta, V = self.update_weights(V, z, R, Q, Q_prime, coded_Q)
            ...
            if episode % 10 == 0:
                ...
                print(f"Śr. liczba kroków: {np.mean(steps):.4f}, Epizod: {episode}, Stan wag: {len(np.unique(V))}, Epsil: {epsilon:.4f}, Akcja: {A:.2f}, Q: {np.mean(Qs):.2f}, Q': {np.mean(Qs_p):.2f}, R: {np.mean(Rs):.2f}, V: {np.mean(V):.2f}, Błąd: {np.mean(deltas):.2f}")
                if episode < 4000:
                    with open(f"lambda_{self.lamb}.txt", "a") as file1:
                        file1.write(f"{np.mean(steps)}\n")
    ...
    def update_weights(self, V, z, R, Q, Q_prime, coded_Q):
        delta = R+self.gamma*Q_prime - Q
        z[coded_Q[0],coded_Q[1],coded_Q[2],coded_Q[3],coded_Q[4]] = self.gamma*self.lamb*z[coded_Q[0],coded_Q[1],coded_Q[2],coded_Q[3],coded_Q[4]]

        V[coded_Q[0],coded_Q[1],coded_Q[2],coded_Q[3],coded_Q[4]]+=self.alpha*delta*z[coded_Q[0],coded_Q[1],coded_Q[2],coded_Q[3],coded_Q[4]]        
        return delta, V
